...
function AccountGate({ onChooseRole }: { onChooseRole: (role: "organizer" | "agent") => void }) {
  const [role, setRole] = useState<"organizer" | "agent">("organizer");
  const beginOAuth = () => { localStorage.setItem("seatroom-pending-role", role); onChooseRole(role); startLogin(); };
  return <div className="account-gate"><div className="account-gate__mark"><Sparkles size={20} /></div><span className="eyebrow">SEATROOM · ACCÈS SÉCURISÉ</span><h1>Votre événement,<br /><em>maîtrisé.</em></h1><p>Connectez-vous pour ouvrir votre espace Organisation ou Accueil.</p><div className="role-switch role-switch--gate"><button className={role === "organizer" ? "active" : ""} onClick={() => setRole("organizer")}><Building2 size={17} /><span><strong>Organisation</strong><small>Listes, QR et pilotage</small></span></button><button className={role === "agent" ? "active" : ""} onClick={() => setRole("agent")}><UserCheck size={17} /><span><strong>Accueil</strong><small>Scanner et vérifier</small></span></button></div><button className="button button--gold button--large" onClick={beginOAuth}>Continuer avec e-mail ou Google <ArrowUpRight size={16} /></button><small className="account-gate__note">L’authentification se fait dans le portail sécurisé.</small></div>;
}

function RoleSetup({ onSelect }: { onSelect: (role: "organizer" | "agent") => void }) {
  const [role, setRole] = useState<"organizer" | "agent">("organizer");
  return <div className="account-gate"><div className="account-gate__mark"><Sparkles size={20} /></div><span className="eyebrow">BIENVENUE DANS SEATROOM</span><h1>Choisissez votre<br /><em>espace.</em></h1><p>Votre rôle définit les fonctions accessibles dans l’application.</p><div className="role-switch role-switch--gate"><button className={role === "organizer" ? "active" : ""} onClick={() => setRole("organizer")}><Building2 size={17} /><span><strong>Organisation</strong><small>Listes, QR et pilotage</small></span></button><button className={role === "agent" ? "active" : ""} onClick={() => setRole("agent")}><UserCheck size={17} /><span><strong>Accueil</strong><small>Scanner et vérifier</small></span></button></div><button className="button button--gold button--large" onClick={() => onSelect(role)}>Ouvrir mon espace <ArrowUpRight size={16} /></button></div>;
}
...
export default function Home() {
  ...
  if (!auth.isAuthenticated) return <AccountGate onChooseRole={() => undefined} />;
